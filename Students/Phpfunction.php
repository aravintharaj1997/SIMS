...
function sec_session_start() {
    $session_name = 'sims_students_login';   // Set a custom session name
...
function login($email, $password, $mysqli) {
    ...
    if ($stmt = $mysqli->prepare("SELECT Student_ID, Username, Password
        FROM students
       WHERE Email_ID = ?
        LIMIT 1")) {
        ...
        if ($stmt->num_rows == 1) {
            ...
            if (checkbrute($user_id, $mysqli) == true) {
                ...
                if (password_verify($password, $db_password)) {
                    ...
                    $prep_stmt="SELECT Department FROM students_details WHERE Student_ID = ? LIMIT 1";
                  ...
                  if($stmt){
                   ...
                   $stmt->fetch();
                    ...
                    $now = time();
                    $mysqli->query("INSERT INTO student_attempts(Student_ID, Time)
                                    VALUES ('$user_id', '$now')");
                    return false;
                }
            }
